...
[![Platform](https://img.shields.io/badge/platform-Windows-0078D6)](#how-to-run)
...
**⬇️ Quick Start**: Download `Lawrie-*-win-x64.exe` from [Releases](https://github.com/SANABI-LL/Lawrie/releases/latest) and double-click to run (Windows).
...
- First run will auto-download the widget (~70MB, one-time), then the rabbit pops up.
- To uninstall: `/plugin uninstall lawrie-pet@lawrie`, hooks are removed cleanly.
- Current widget is for **Windows**; see "Manual Setup / Other Platforms" below for macOS / Linux.
...
**All 8 states complete** 🎉: MVP set (idle/typing/thinking/sleeping/happy) + error + notification + carrying.
...
`runtime/widget/` is a minimalist custom Electron widget (transparent, always-on-top, native dragging, corner-resize, size memory) that turns the 8 `.svg.html` states into a **desktop pet that reacts to Claude Code in real-time**.
...
> **Platform Note**: Pre-packaged exe and the hook launch command (`explorer.exe`) are for **Windows**.
> **macOS / Linux** users can run from source: `npm start` directly shows the widget (Electron is cross-platform); to integrate with Claude Code, edit `hooks.example.json` `SessionStart` launch command to your system equivalent — macOS use `open -a "/path/Lawrie.app" || exit 0`, Linux write executable path `/path/lawrie &` (other `curl` lines work cross-platform). To package for mac/linux, add `mac` / `linux` target alongside `build.win` in `runtime/widget/package.json` then run `npm run dist`.
...
|---|---|
| SessionStart | idle (launches widget if not running) |
...
{
  "hooks": {
    "SessionStart":       [{ "matcher": "", "hooks": [{ "type": "command", "command": "curl -s -m 1 http://127.0.0.1:4747/idle || explorer.exe \"C:\\Users\\YourUsername\\Lawrie\\Lawrie.exe\" || exit 0" }] }],
